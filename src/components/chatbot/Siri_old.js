...
const PerformanceManager = {
  ...
  shouldUseFlatMode() {
    ...
    // Use more lenient limits for mobile to default to 3D more often
    const memoryLimit = isMobile ? 2 : 4; // GB
    const cpuCoreLimit = isMobile ? 2 : 4;

    try {
      // Reduced motion preference is a strong signal to use flat mode
      if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
        return true;
      }

      // For non-mobile, we can be more aggressive with performance checks
      if (!isMobile) {
        // Memory check
      ...
      // For mobile, only use flat mode if memory is very low.
      // This makes 3D the default for most modern phones.
      // Memory check
      if (
        isMobile &&
        navigator.deviceMemory &&
        navigator.deviceMemory < memoryLimit
      ) {
        return true;
      }
  ...
  const getAIResponse = useCallback(
    async (userInput) => {
      try {
        ...
        const contextPrompt = `You are Laxman's enhanced AI assistant. Respond naturally and engagingly in ${maxWords} words or less:
...
Provide an engaging, conversational response:`;
  ...
  const speakIntroduction = useCallback(async () => {
    const introText = deviceInfo.isMobile
      ...
      : "Hey! I'm Laxman's AI assistant. How can I help you discover Laxman's expertise?";
    ...
    async (text) => {
      ...
      try {
        ...
        setTimeout(() => {
          ...
          const timeoutDuration = Math.max(cleanText.length * 100, 5000); // At least 5 seconds
          speechTimeoutRef.current = setTimeout(() => {
            if (synthRef.current) {
  ...
  const processFinalSpeech = useCallback(
    async (finalText) => {
      ...
      console.log("🧠 Processing speech input:", cleanedText);

      if (cleanedText.length < 2 || isProcessingRef.current) {
        console.warn(
          "🧠 Skipping processing - text too short or already processing"
        );
        return;
      }
      ...
      try {
        console.log("🧠 Getting AI response for:", cleanedText);
      ...
      if (!puterReady) {
        ...
        if (retryCount < 3) {
          ...
          showNotice(
            "error",
            "🤖 AI Service",
            "AI service unavailable. Please refresh the page."
          );
        }
        return;
      }
      ...
      >
        ...
        >
          ...
          {deviceInfo.isMobile
            ...
            : "Experience enhanced voice - ask about Laxman's expertise!"}
        </HelpHint>
      </VoiceContainer>
    </>
  );
